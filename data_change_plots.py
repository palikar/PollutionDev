...
def change_plot(folds, dest):
    ...
    for folder in folds:
        ...
        plt.title("Min value comparison of PM10")        
        ...
        plt.xlabel('Sensor ID', fontsize=9)
        plt.title("Max value comparison of PM10")
        plt.xlim(min(index)-bar_width*2, max(index)+bar_width*2)
        ...
        plt.xlabel('Sensor ID', fontsize=9)
        plt.title("Mean value comparison of PM10")
        plt.xlim(min(index)-bar_width*2, max(index)+bar_width*2)
        ...
        plt.xlabel('Sensor ID', fontsize=9)
        plt.title("Std comparison of PM10")
        plt.xlim(min(index)-bar_width*2, max(index)+bar_width*2)
        ...
        plt.ylabel("Min value")
        plt.xlabel('Sensor ID', fontsize=9)
        plt.title("Min value comparison of PM2.5")        
        plt.xlim(min(index)-bar_width*2, max(index)+bar_width*2)
        plt.ylim([0, max(np.append(before_df["min"].values, after_df["min"].values))*1.2])
        plt.legend()
        ...
        plt.ylabel("Max value")
        plt.xlabel('Sensor ID', fontsize=9)
        plt.title("Max value comparison of PM2.5")
        plt.xlim(min(index)-bar_width*2, max(index)+bar_width*2)
        plt.ylim([0, max(np.append(before_df["max"].values, after_df["max"].values))*1.2])
        plt.legend()
        ...
        plt.ylabel("Mean value")
        plt.xlabel('Sensor ID', fontsize=9)
        plt.title("Mean value comparison of PM2.5")
        plt.xlim(min(index)-bar_width*2, max(index)+bar_width*2)
        plt.ylim([0, max(np.append(before_df["mean"].values, after_df["mean"].values))*1.2])
        plt.legend()
        ...
        plt.ylabel("Std value")
        plt.xlabel('Sensor ID', fontsize=9)
        plt.title("Std comparison of PM2.5")
        plt.xlim(min(index)-bar_width*2, max(index)+bar_width*2)
        plt.ylim([0, max(np.append(before_df["std"].values, after_df["std"].values))*1.2])
        plt.legend()
...
def main():
    ...
    parser.add_argument('folders', metavar='FOLDs',
                        help='The main folders to process. These folders must point to the root of that what comes out of the archives with the generated data.', nargs='+')
